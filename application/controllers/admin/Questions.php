...
class Questions extends Admin_Controller {
 ...
 // die("OK");
        // get list
        $questions = $this->questions_model->get_all($limit, $offset, $filters, $sort, $dir);

        // build pagination
        $this->pagination->initialize(array(
            'base_url'   => THIS_URL . "?sort={$sort}&dir={$dir}&limit={$limit}{$filter}",
            'total_rows' => $questions['total'],
            'per_page'   => $limit
        ));
		...
		$this
			...
			->set_title( lang('questions title questions_list') );
    ...
    function edit($id = NULL)
    {
        ...
        // setup page header data
        $this->set_title( lang('questions title questions_edit') );
    ...
    function delete($id = NULL)
    {
        ...
        {
            // get user details
            $user = $this->questions_model->get_user($id);

            if ($user)
            {
                // soft-delete the user
                $delete = $this->questions_model->delete_user($id);

                if ($delete)
                {
                    $this->session->set_flashdata('message', sprintf(lang('questions msg delete_question'), $user['first_name'] . " " . $user['last_name']));
                }
                else
                {
                    $this->session->set_flashdata('error', sprintf(lang('questions error delete_question'), $user['first_name'] . " " . $user['last_name']));
                }
            }
            else
            {
